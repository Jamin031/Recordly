...
type Props = ComponentProps<typeof VideoPlayback> & {
 ...
 *
 * This keeps the large VideoPlayback renderer untouched while the layout
 * timeline is introduced. Legacy callers (layoutEvents === undefined) are
 * forwarded byte-for-byte. Opted-in projects render the webcam in this
 * wrapper so Presenter and Screen/PIP can share the same deterministic layout
 * resolver used by export.
 */
export function LayoutVideoPlayback({
	layoutEvents,
	webcam,
	webcamVideoPath,
	currentTime,
	...
	useEffect(() => {
		...
		if (!isPlaying && Math.abs(video.currentTime - target) > 0.03) {
			try {
				...
				// Media metadata may not be ready yet; the next timeline update retries.
			}
		}
